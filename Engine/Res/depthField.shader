	...
	Pass {
		...
		SHADER_BEGIN: [ d3d10 d3d11 ]
			...
			#code [
				...
				SamplerState screenSampleType;

				Texture2D blurTex;
				SamplerState blurSampleType;
				...
				{
				    float4 textureColor = screenTexture.Sample(screenSampleType, input.tex);
				    float4 blurColor = blurTex.Sample(blurSampleType, input.tex);
				    float depth = g_engineDepthTexture.Sample(SampleType, input.tex).r;

				    depth = saturate((depth - area.x)/(area.y - area.x));

				    textureColor.rgb = lerp(textureColor.rgb, blurColor.rgb, depth);
		...
		SHADER_BEGIN: [ d3d9 ]
			...
			#code [
				...
				float4 FragMain(VS_OUTPUT input) : SV_TARGET
				{
	    			...
	    			float4 blurColor = tex2D(blurTex, input.uv);
	    			float depth = tex2D(g_engineDepthTexture, input.uv).r;
