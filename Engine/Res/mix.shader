	...
	Pass {
		...
		SHADER_BEGIN: [ d3d10 d3d11 ]
			...
			#code [
				...
				{
				    float4 textureColor = screenTexture.Sample(SampleType, input.tex);
				    float4 mixColor = mix.Sample(mixSampler, input.tex);

				   textureColor.rgb += mixColor.rgb*2;
				   textureColor.a = 1.0f;
